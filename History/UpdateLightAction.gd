	...
	if _node.definition.has("export_index"):
		...
		for property_name in _new_properties:
			if _old_properties.has(property_name):
				if _old_properties[property_name] == null:
					_node.definition.erase(property_name)
					update_properties[property_name] = _node.get_light_default(property_name)
				...
				_node.definition.erase(property_name)
				update_properties[property_name] = _node.get_light_default(property_name)
				Editor.set_room_edit_export_prop(_node.tree_name, export_index, property_name,  _node.get_light_default(property_name))
